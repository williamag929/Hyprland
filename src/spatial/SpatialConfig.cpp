...
namespace Spatial {
    ...
    bool SpatialConfig::parseConfigSection(const std::string& configContent) {
        ...
        while (std::getline(iss, line)) {
            ...
            try {
                ...
                std::cerr << "[SpatialConfig] Invalid value for '" << key << "' (\"" << value << "\") — keeping default. (" << e.what() << ")\n";
            } catch (const std::out_of_range& e) {
                std::cerr << "[SpatialConfig] Value out of numeric range for '" << key << "' (\"" << value << "\") — keeping default. (" << e.what() << ")\n";
            }
        }
    ...
    void SpatialConfig::validateAndClamp() {
        ...
        if (m_iZLayerCount < 1 || m_iZLayerCount > 16) {
            std::cerr << "[SpatialConfig] z_layers=" << m_iZLayerCount << " is out of valid range [1,16], clamping\n";
        ...
        if (!std::isfinite(m_fZLayerStep) || m_fZLayerStep <= 0.0f || m_fZLayerStep > 100000.0f) {
            std::cerr << "[SpatialConfig] z_layer_step=" << m_fZLayerStep << " must be a finite value in (0, 100000], resetting to default 800\n";
        ...
        if (!std::isfinite(m_fZAnimationStiffness) || m_fZAnimationStiffness < 1.0f) {
            std::cerr << "[SpatialConfig] z_animation_stiffness=" << m_fZAnimationStiffness << " must be a finite value >= 1.0, clamping\n";
        ...
        if (!std::isfinite(m_fZAnimationDamping) || m_fZAnimationDamping < 0.0f) {
            std::cerr << "[SpatialConfig] z_animation_damping=" << m_fZAnimationDamping << " must be a finite value >= 0.0, clamping\n";
        ...
        if (!std::isfinite(m_fZFOVDegrees) || m_fZFOVDegrees <= 0.0f || m_fZFOVDegrees >= 180.0f) {
            std::cerr << "[SpatialConfig] z_fov_degrees=" << m_fZFOVDegrees << " must be a finite value in (0, 180), resetting to default 60\n";
        ...
        if (!std::isfinite(m_fZNearPlane) || m_fZNearPlane <= 0.0f) {
            std::cerr << "[SpatialConfig] z_near_plane=" << m_fZNearPlane << " must be a finite value > 0, resetting to default 0.1\n";
            m_fZNearPlane = 0.1f;
        }

        // z_far_plane: must be finite and strictly greater than near
        if (!std::isfinite(m_fZFarPlane) || m_fZFarPlane <= m_fZNearPlane) {
            std::cerr << "[SpatialConfig] z_far_plane=" << m_fZFarPlane << " must be a finite value > z_near_plane (" << m_fZNearPlane << "), resetting to default 10000\n";
