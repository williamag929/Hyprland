...
namespace Spatial {
    ...
    ZSpaceManager::ZSpaceManager() :
        m_iActiveLayer(0), m_fCameraZ(0.0f), m_fCameraZTarget(0.0f), m_fCameraZVelocity(0.0f), m_fCurrentFov(Z_FOV_DEGREES), m_iScreenW(0), m_iScreenH(0) {}
    ...
    void ZSpaceManager::removeWindow(void* window) {
        ...
        auto                                  it = std::find_if(m_vWindowsZ.begin(), m_vWindowsZ.end(), [window](const WindowZ& wz) { return wz.pWindow == window; });
    ...
    glm::mat4 ZSpaceManager::getSpatialView() const {
        ...
        return glm::lookAt(glm::vec3(m_iScreenW / 2.0f, m_iScreenH / 2.0f, camZ + 1200.0f), // eye
    ...
    std::vector<void*> ZSpaceManager::getSortedWindowsForRender() const {
        ...
        std::sort(sorted.begin(), sorted.end(), [](const WindowZ* a, const WindowZ* b) {
    ...
    void ZSpaceManager::debugPrint() const {
        std::lock_guard<std::recursive_mutex> lock(m_mtxWindows);
        std::cout << "[ZSpaceManager] active_layer=" << m_iActiveLayer << "  camera_z=" << m_fCameraZ << "  windows=" << m_vWindowsZ.size() << "\n";
        for (const auto& wz : m_vWindowsZ) {
            std::cout << "  win=" << wz.pWindow << "  layer=" << wz.iZLayer << "  z=" << wz.fZPosition << "  target=" << wz.fZTarget << "  vel=" << wz.fZVelocity
                      << (wz.bZPinned ? "  [PINNED]" : "") << "\n";
        }
        std::cout.flush();
    }
